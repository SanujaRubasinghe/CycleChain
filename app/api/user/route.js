...
import Ride from "@/models/ride";         // created above
...
export async function GET() {
  ...
  // Usage summary
  const summaryAgg = await Ride.aggregate([
    { $match: { user: user._id } },
    {
      $group: {
        _id: null,
  ...
  const summary = summaryAgg[0] || { totalRides: 0, totalDistance: 0, totalCost: 0, lastRideAt: null };
